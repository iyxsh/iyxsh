<template>
  <div class="style-manager">
    ...
    <div class="style-manager-content">
      <div v-for="category in allStylesTree" :key="category.id" class="toolbar-category">
        ...
        <div class="toolbar-icons">
          <div v-for="item in category.children" :key="item.id" class="toolbar-item"
            :class="{ active: isVisibleStyle(item.key) }" @click.stop="handleToolbarItemClick($event, item)"
    ...
    <!-- 样式设置对话框 -->
    <el-dialog v-model="styleSettingsDialogVisible" :title="currentSettingItem?.label" width="400px">
      <div v-if="currentSettingItem" class="setting-dialog-content">
        <template v-if="currentSettingItem.type === 'number'">
          <el-input-number v-model="localStyle[currentSettingItem.key]" :min="currentSettingItem.min"
            :max="currentSettingItem.max" :step="currentSettingItem.step" size="small" controls-position="right"
            @change="updateStyle" />
        ...
        <template v-else-if="currentSettingItem.type === 'slider'">
          <el-slider v-model="localStyle[currentSettingItem.key]" :min="currentSettingItem.min"
            :max="currentSettingItem.max" :step="currentSettingItem.step" size="small"
            :show-input="currentSettingItem.showInput" @change="updateStyle" />
          <span v-if="currentSettingItem.showValue" class="value-display">{{ localStyle[currentSettingItem.key] }}{{
            currentSettingItem.unit || '' }}</span>
        </template>

        <template v-else-if="currentSettingItem.type === 'color-picker'">
          <el-color-picker v-model="localStyle[currentSettingItem.key]" size="small"
            :show-alpha="currentSettingItem.showAlpha" @change="updateStyle" />
        </template>

        <template v-else-if="currentSettingItem.type === 'select'">
          <el-select v-model="localStyle[currentSettingItem.key]" size="small" @change="updateStyle">
            <el-option v-for="option in currentSettingItem.options" :key="option.value" :label="option.label"
              :value="option.value" />
          </el-select>
        </template>
        ...
        <template v-else-if="currentSettingItem.type === 'button-group'">
          <div class="button-group">
            <el-button v-for="btn in currentSettingItem.buttons" :key="btn.value" size="small"
        ...
        <template v-else-if="currentSettingItem.type === 'input'">
          <el-input v-model="localStyle[currentSettingItem.key]" size="small" @change="updateStyle" />
        </template>

        <template v-else-if="currentSettingItem.type === 'switch'">
          <el-switch v-model="localStyle[currentSettingItem.key]" :active-value="currentSettingItem.activeValue"
            :inactive-value="currentSettingItem.inactiveValue" @change="updateStyle" />
